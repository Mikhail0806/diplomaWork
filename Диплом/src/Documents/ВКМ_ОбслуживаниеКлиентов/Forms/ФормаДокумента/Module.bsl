...
Процедура ПриОткрытии(Отказ)
    ...
    Если Не ЗначениеЗаполнено(Объект.ДатаПроведенияРабот) Тогда  
		...
		Объект.ДатаПроведенияРабот = Объект.Дата;
...
Функция РаботаСоВременем(ДатаПроведения, ВремяПроведения)
	...
	ДатаВремя = Дата(Год(ДатаПроведения), Месяц(ДатаПроведения), День(ДатаПроведения), Час(ВремяПроведения), Минута(
		ВремяПроведения), Секунда(ВремяПроведения));
	Возврат ДатаВремя;
...
Процедура СозданиеЭлементаСправочника()
	...
	Для Каждого Строка Из ИзменениеРеквизитов Цикл
		...
		СтрокаСообщения = СтрШаблон("Реквизит ""%1"" изменен: %2", Строка.Реквизит, Строка.Изменение);
	...
	Если Не ПустаяСтрока(ТекстСообщения) Тогда
		ТекстСообщения = "Уведомление об изменениях от " + ТекущаяДатаСеанса() + " : " + Символы.ПС + ТекстСообщения;
	КонецЕсли;
