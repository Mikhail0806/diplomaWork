...
Запрос = Новый Запрос;
	...
	|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОстатки.Договор, 0) КАК Договор
	|ИЗ
	|	РегистрНакопления.ВКМ_ВыполненныеКлиентуРаботы.Остатки(&Дата, Договор = &Договор) КАК ВКМ_ВыполненныеКлиентуРаботыОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОстатки.Договор, 0)";
